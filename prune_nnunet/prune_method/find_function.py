...
def get_process_function(class_name):
    """
    Get the apply function for a given pruning method class.
    ...
    try:
        ...
        # Find the function that starts with "apply_"
        for name, obj in inspect.getmembers(module):
            if inspect.isfunction(obj) and name.startswith("process_"):
                return obj

        raise ValueError(f"No 'process_' function found in {module_path}")

    except ImportError:
        raise ImportError(f"Could not import module {module_path}")


def get_save_data_function(class_name):
    """
    Get the apply function for a given pruning method class.
    ...
    try:
        ...
        # Find the function that starts with "apply_"
        for name, obj in inspect.getmembers(module):
            if inspect.isfunction(obj) and name.startswith("save_"):
                return obj

        raise ValueError(f"No 'save_' function found in {module_path}")
